        ...
        <table className="w-full border border-gray-300 mt-4 mb-2">
          <thead>
            <tr className="bg-gray-200">
              {
                ['ID', 'Title', 'Category', 'Author', 'Downloads', 'Created At', 'Actions'].map((item, index) => {
          ...
          <tbody>
            {data.data.map((book) => (
              <tr key={book.id} className="text-center">
                ...
                <td className="p-2 border">novel</td>
                <td className="p-2 border">{book.author}</td>
                <td className="p-2 border">2</td>
                <td className="p-2 border">2-2-2025</td>
                <td className="p-2 border">
                  <div className="flex justify-center">
                    <Link to={`${book.id}/edit`} className="p-2 text-sm me-2 text-gray-100 bg-blue-600 hover:bg-blue-700 delay-75 duration-150 rounded-md">Edit</Link>
                    <Link to={`/books/${book.id}`} className="p-2 text-sm me-2 text-gray-100 bg-red-600 hover:bg-red-700 delay-75 duration-150 rounded-md">Delete</Link>
                  </div>
                </td>
              </tr>
            ))}

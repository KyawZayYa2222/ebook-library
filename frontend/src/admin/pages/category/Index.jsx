...
export default function Categories() {
  ...
  const [url, setUrl] = useState(`http://localhost:3000/categories?_page=${currentPage}&_per_page=${limit}`)

  const { data, loading, error } = useFetch(url, "GET", true);

  const onPageChange = (page) => {
    setCurrentPage(page)
    setUrl(`http://localhost:3000/categories?_page=${page}&_per_page=${limit}`)
  }
        ...
        <table className="w-full border border-gray-300 mt-4 mb-2">
          <thead>
            <tr className="bg-gray-200">
              {
                ['ID', 'Title', 'Books', 'Created At', 'Actions'].map((item, index) => {
              ...
              <tr key={category.id} className="text-center">
                ...
                <td className="p-2 border">
                  <div className="flex justify-center">
                    <Link to={`${category.id}/edit`} className="p-2 text-sm me-2 text-gray-100 bg-blue-600 hover:bg-blue-700 delay-75 duration-150 rounded-md">Edit</Link>
                    <Link to={`/categories/${category.id}`} className="p-2 text-sm me-2 text-gray-100 bg-red-600 hover:bg-red-700 delay-75 duration-150 rounded-md">Delete</Link>
                  </div>
                </td>
              </tr>
            ))}
